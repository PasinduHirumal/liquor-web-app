...
// UPDATE VALIDATOR - NO defaults, all fields optional
const validateCommissionRateUpdate = (req, res, next) => {
  if (!req.body || Object.keys(req.body).length === 0) {
    return res.status(400).json({ success: false, message: "Validation failed"});
  }
  
  const schema = Joi.object({
    commissionRate_drivers: Joi.number().min(0).max(100).required(),
  })
  .min(1) // Require at least one field to update
  .options({ stripUnknown: true });; 
...
export { validateAppInfoUpdate, validateLiquorActiveToggleUpdate, validateCommissionRateUpdate };